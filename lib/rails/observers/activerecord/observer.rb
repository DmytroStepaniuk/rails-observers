  ...
  class Observer < ActiveModel::Observer
    ...
    protected
      ...
      def define_callbacks klass
        ...
        observerd_callbacks.each do |callback|
          next unless respond_to? callback

          callback_meth = :"_notify_#{ observer_name }_for_#{ callback }"

          unless klass.respond_to?(callback_meth)
            klass.send(:define_method, callback_meth) { |&block| observer.update(callback, self, &block) }
      ...
      def observerd_callbacks
        ActiveRecord::Callbacks::CALLBACKS + [:after_create_commit, :after_update_commit, :after_destroy_commit]
      end
  end
end
